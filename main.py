...
FPS = 60
SHOW_MINICAM = False 
PLAY_THROUGH_MIC = False
GAME_WIDTH = 1280

# global constant variables
GAME_HEIGHT = round(GAME_WIDTH * 0.5625)
WANTED_WIDTH, WANTED_HEIGHT = 1920, 1080 
...
fruit_slice_sounds = [
    ...
    "assets/sounds/splatter_3.wav",
    "assets/sounds/splatter_4.wav",
]
...
MAIN_NUMBER_FONT = pygame.font.Font('assets/fonts/minecraft_font.ttf', ratio(50))
TITLE_FONT = pygame.font.Font('assets/fonts/minecraft_font.ttf', ratio(60))
...
def main():
    ...
    with MP_POSE.Pose(
        ...
        main_music = play_sound("assets/sounds/music.wav")
        ...
        while cap.isOpened() and running:
            ...
            results, image_to_display = detection.find_and_draw_pose(
                pose, 
                frame,
                frame)

            if SHOW_MINICAM:
                detection.add_webcam_feed(image_to_display, frame)
            
            # transform image_to_display into a pygame surface
            cam_img = detection.array_img_to_pygame(image_to_display, GAME_WIDTH, GAME_HEIGHT)
            screen.fill(RED)
            ...
            left_hand, right_hand = detection.knife_trails_and_find_hands(
                results, 
                left_knife_trail, 
                right_knife_trail, 
                GAME_WIDTH, 
            ...
            if len(left_knife_trail) >= 2 and start_game == False or dead_fruit:
                for i, point in enumerate(left_knife_trail):
                    ...
                    if time_painted + KNIFE_TRAIL_LIFETIME > time.time(): 
                        ...
                        if len(left_knife_trail) - 1 > i:             
                            left_circles += detection.knife_trail(
                                screen, 
                                WHITE, 
                                coords, 
                ...
                for i, point in enumerate(right_knife_trail):
                    ...
                    if time_painted + KNIFE_TRAIL_LIFETIME > time.time():
                        if len(right_knife_trail) - 1 > i:
                            right_circles += detection.knife_trail(
                                screen, 
                                WHITE, 
                                coords, 
            ...
            if type(start_fruit) is Fruit: 
                ...
                if destroy:
                    ...
                    play_sound("assets/sounds/game start.wav", 0.5)
                ...
                start_fruit = Fruit( # create fruit object
                    name="Start Fruit", 
                    img_filepath="assets/images/subscribe.png", 
                    starting_point=(475,400),
                    size=(300,300),
                    velocity=0,
                    points=0)
                last_round = time.time()
                ...
                if dead_fruit is None:
                    ...
                    play_sound("assets/sounds/death sound.wav", 0.6)
                ...
                explosion.fill((80, 0, 0, explosion_alpha))
                screen.blit(explosion, (0, 0))

                lost_text = TITLE_FONT.render('You Died!', False, WHITE)
                lost_text2 = TITLE_FONT.render('', False, WHITE)
                screen.blit(lost_text, (
                    round(GAME_WIDTH / 2) - lost_text.get_width() / 2, 
                    round(GAME_HEIGHT / 3) - lost_text.get_height() / 2))

                screen.blit(lost_text2, (335, 300))
                ...
                if destroy:
                    ...
                    explosion_alpha = 0
                    play_sound(random.choice(fruit_slice_sounds))
                    play_sound("assets/sounds/game start.wav")
...
def draw_start_end_fruit(
    ...
    if time.time() - last_round >= ROUND_COOLDOWN: 
        for left_circle in left_circles:
            if detection.distance_2D(fruit.get_centre(), left_circle) <= circle_radius:
                hit_left = True
                break

        if hit_left: # must hit with both hands!
            for right_circle in right_circles:
                if detection.distance_2D(fruit.get_centre(), right_circle) <= circle_radius:
                    return True
    """
    pygame.draw.circle(
        screen,
...
def display_menu(fps, start_game, round_number=None, total_points=None, fruits_left=None):
    ...
    if type(round_number) is int:
        ...
        starting_title = TITLE_FONT.render(f'', False, NINJA_RED) #SMASH SUBSCRIBE BUTTON
...
def fruit_hit_circles(fruit: Fruit, points: list):
    for point in points:
        if fruit.rect.collidepoint(point): # check if it's in rectangle first
            if detection.colliding_fruit(point, fruit):
                return True
    return False
...
def process_fruit(
    ...
    # if fruit is finally on screen and we didn't play its spawn sound yet
        ...
        if fruit.name == "bomb":
            ...
            play_sound("assets/sounds/fruit spawn.wav")

        fruit.spawn_sound_played = True

    if fruit.name == "bomb":
        if (fruit.rect.collidepoint(left_hand)
            or fruit.rect.collidepoint(right_hand)):
                bomb_touched = True
                play_sound("assets/sounds/explode.wav")
    else:
    ...
    # if reached max_fruit_height, go down
